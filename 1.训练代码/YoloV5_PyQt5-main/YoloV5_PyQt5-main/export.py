...
def export_formats():
    ...
    return pd.DataFrame(x, columns=['Format', 'Argument', 'Suffix', 'CPU', 'GPU'])#导出格式，是否支持cpu和gpu


def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):#导出yolov5模型为TorchScript模式
    ...
    try:
        ...
        extra_files = {'config.txt': json.dumps(d)}  # 将额外的信息保存到配置文件中
...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    try:
        ...
        torch.onnx.export(
            model.cpu() if dynamic else model,  #动态模式只兼容CPU
        ...
        # 加载一个ONNX模型文件，并对加载的模型进行规范性检查，确保模型的正确性和可用性
        model_onnx = onnx.load(f)  # load onnx model
        onnx.checker.check_model(model_onnx)  # check onnx model

        # 将一些模型的元数据信息添加到已加载的ONNX模型中，并将更新后的模型保存回指定的ONNX文件中
...
def export_edgetpu(file, prefix=colorstr('Edge TPU:')):
    # YOLOv5 Edge TPU export TPU编译器进行优化
    try:
...
def run(
    ...
    # Input
    gs = int(max(model.stride))  #计算模型中最大步长，并将其转换为整数，用于指定在特征图上滑动窗口的步幅大小。
    imgsz = [check_img_size(x, gs) for x in imgsz]  #对输入的图像尺寸列表imgsz中的每个尺寸进行检查，确保它们是网格大小的整数倍。
    im = torch.zeros(batch_size, 3, *imgsz).to(device)  #创建一个全零张量作为输入图像，3表示通道数
    ...
    #根据不同的条件选择性地将PyTorch模型导出为多种不同的格式，以便在不同的部署环境中使用或部署
